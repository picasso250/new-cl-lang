...
def emit_gc() -> list[str]:
    return [
        ...
        "static struct { void** slot; int active; }* __nc_gc_roots = NULL;",
        "static size_t __nc_gc_root_n = 0;",
        "static size_t __nc_gc_root_cap = 0;",
        "static nc_record_t** __nc_gc_gray = NULL;",
        "static size_t __nc_gc_gray_top = 0;",
        "static size_t __nc_gc_gray_cap = 0;",
        "static void __nc_gc_abort_oom(void) { fprintf(stderr, \"nc gc: out of memory\\n\"); abort(); }",
        ...
        "    return NULL; }",
        "static void __nc_gc_gray_push(nc_record_t* r) {",
        "    if (__nc_gc_gray_top >= __nc_gc_gray_cap) {",
        "        size_t nc = __nc_gc_gray_cap ? __nc_gc_gray_cap * 2 : 4096;",
        "        nc_record_t** np = (nc_record_t**)realloc(__nc_gc_gray, nc * sizeof(nc_record_t*));",
        "        if (!np) __nc_gc_abort_oom();",
        "        __nc_gc_gray = np; __nc_gc_gray_cap = nc; }",
        "    __nc_gc_gray[__nc_gc_gray_top++] = r; }",
        "static void __nc_gc_mark_gray(void* p) {",
        "    nc_record_t* r = __nc_gc_find(p);",
        "    if (r && r->marked == 0) { r->marked = 1; __nc_gc_gray_push(r); } }",
        ...
        "    void* p = calloc(1, sz); if (!p) __nc_gc_abort_oom();",
        "    nc_record_t* rec = (nc_record_t*)malloc(sizeof(nc_record_t));",
        "    if (!rec) __nc_gc_abort_oom();",
        "    rec->ptr = p; rec->size = sz; rec->marked = 0;",
        "    rec->next = __nc_gc_registry; __nc_gc_registry = rec; return p; }",
        "static int __nc_gc_push_root_slot(void* slot) {",
        "    if (__nc_gc_root_n >= __nc_gc_root_cap) {",
        "        size_t nc = __nc_gc_root_cap ? __nc_gc_root_cap * 2 : 256;",
        "        void* np = realloc(__nc_gc_roots, nc * sizeof(*__nc_gc_roots));",
        "        if (!np) __nc_gc_abort_oom();",
        "        __nc_gc_roots = np; __nc_gc_root_cap = nc; }",
        "    __nc_gc_roots[__nc_gc_root_n].slot = (void**)slot;",
        "    __nc_gc_roots[__nc_gc_root_n].active = 1;",
        "    return (int)__nc_gc_root_n++; }",
        "static void __nc_gc_set_root(int h, void* p) { if (h>=0 && h<(int)__nc_gc_root_n && __nc_gc_roots[h].slot) *__nc_gc_roots[h].slot = p; }",
        ...
        "    for (size_t i = 0; i < __nc_gc_root_n; i++) if (__nc_gc_roots[i].active && __nc_gc_roots[i].slot) __nc_gc_mark_gray(*__nc_gc_roots[i].slot);",
...
def emit_map() -> list[str]:
    return [
        ...
        "    m->entries = 0; m->cap = 0; m->len = 0; m->tombstones = 0;",
        "}",
        "",
...
def emit_gc_helpers() -> list[str]:
    return [
        ...
        "    __nc_gc_registry = NULL; __nc_gc_root_n = 0; __nc_gc_root_cap = 0; __nc_gc_gray_top = 0; __nc_gc_gray_cap = 0;",
        "    free(__nc_gc_roots); __nc_gc_roots = NULL; free(__nc_gc_gray); __nc_gc_gray = NULL; }",
        "",

...
- `go` 是语句，只接函数调用（含闭包），不返回值。
- 调度器实现为 ncrt C runtime：M 个 OS worker（M = NumCPU）从全局 run queue 取 green thread 执行。
- 每个 green thread 固定 64KB 栈；上下文切换用汇编 save/restore callee-saved registers + %rsp。
- 合作式 yield：编译器在每个函数入口插入时间片检查，超时则 yield。
- main 等待所有 green thread 退出后才 exit。
- 同步用标准库 `sync.Mutex`；阻塞模拟用 `sleep`（timer yield）。
- GC STW 通过窄化异步抢占（信号/APC）实现。
- v1 不做 channel/select、TCP netpoller、工作窃取、动态栈增长。
- 纯计算热循环（无函数调用）不 yield，仅卡住一个 worker。

why：并发是现代系统语言的核心能力；Go 的 goroutine 模型经过工业验证；case 驱动最小 v1 先覆盖 go + mutex + sleep + 死锁检测，后续由真实需求推动 I/O 和 channel。

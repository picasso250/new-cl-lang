...
我们要 Go 风格的 N:M 绿色线程（green thread），由 `runtime.spawn(fun() { ... })` 标准库函数启动。`runtime.spawn` 通过编译器洞实现：typecheck 阶段识别后将闭包 lowering 为 green thread 创建路径。

- `runtime.spawn` 是标准库函数，不是关键字。接受一个 `fun()` 闭包（零参数任意返回类型，返回值丢弃），不阻塞调用端。
...
- v1 不做 `go` 关键字（未来可作为 `runtime.spawn` 的语法糖）、channel/select、TCP netpoller、工作窃取、动态栈增长。

why：并发是现代系统语言的核心能力；Go 的 goroutine 模型经过工业验证；case 驱动最小 v1 先覆盖 runtime.spawn + mutex + sleep + 死锁检测 + 合作式 GC，后续由真实需求推动 I/O 和 channel。`runtime.spawn` 无需语言关键字即可工作，使每个 green thread 创建显式、有意图、不廉价。未来可用 `go f()` 脱糖追加简洁性。

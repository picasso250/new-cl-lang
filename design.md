...
- LLVM 后端 v1 当前承诺基础闭环：基础数值/bool 类型、`str` 字面量、数值转换、`len(str)`、`str ==/!=`、定长数组字面量/索引/索引赋值、struct 值类型声明/字面量/字段读写/参数与返回、算术/比较、`let`、重赋值、函数、显式 `return` 与尾表达式返回、`if`、条件 `for`、函数调用与 `io.println`。

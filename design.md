...
- C/LLVM 后端共享 `runtime/ncrt.h` + `runtime/ncrt.c` 编译出的 `ncrt.obj`。`ncrt` 固定基础 ABI：`str`、`nc_map`、`nc_slice_raw`、`__nc_gc_alloc`/`__nc_gc_collect`/`__nc_gc_live`、root slot 兼容 stub、字符串/file/cast/map helper、字节级 slice append/copy helper 与 C 异常入口。C 生成代码只 include `ncrt.h`，不再内联共享 runtime；`[]T` 语言布局仍为 `{ T* ptr; u64 len; u64 cap }`，`elem_size` 仅作为 runtime helper 调用参数传入，不进入 slice header。

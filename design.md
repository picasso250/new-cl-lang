...
我们要 Go 风格的 N:M 绿色线程（green thread），由 `runtime.spawn(fun() { ... })` 内建特殊函数启动。编译器在 name resolution 后按 `BuiltinRuntimeSpawn` 符号身份识别，不走文本名字匹配。

- `runtime.spawn` 不是一等函数值，v1 只允许直接调用形式。接受一个 `fun()` 闭包（零参数任意返回类型，返回值永远丢弃）。
- 编译器 lowering 只生成 `__nc_spawn(fn_ptr, env_ptr)`，G 分配、计数、run queue、worker wake 全由 ncrt 内部完成。
- `main` 本身也是一个 green thread（live_count 初始为 1），进程在 main 结束且所有用户 green thread 退出后才 exit。
- 调度器实现为 ncrt C runtime：M 个 OS worker（M = NumCPU）从全局 run queue 取 green thread 执行。run queue enqueue 和 timer 插入更早 deadline 时均 signal sleeping worker（condition variable）。
...
- 同步用标准库 `sync.Mutex`（handoff ownership + `park_current_and_unlock` 原语，消除丢唤醒窗口）。
...
- v1 不做 `go` 关键字（未来可作为语法糖脱糖）、channel/select、TCP netpoller、工作窃取、动态栈增长、loop backedge 时间片 yield。

why：并发是现代系统语言的核心能力；Go 的 goroutine 模型经过工业验证；case 驱动最小 v1 先覆盖 runtime.spawn + mutex + sleep + 死锁检测 + 合作式 GC；`runtime.spawn` 无需关键字即可工作，每个 spawn 写闭包使创建显式、有意图、不廉价。未来可用 `go f(args)` 脱糖追加简洁性。

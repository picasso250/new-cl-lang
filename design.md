...
我们要 Go 风格的 N:M 绿色线程（green thread），由 `spawn fun() { ... }` 关键字启动。

- `spawn` 是关键字，后跟一个 `fun()` 闭包表达式（零参数任意返回类型。返回值永远丢弃）。`spawn` 是语句，不阻塞调用端。
...
- 同步用标准库 `sync.Mutex`（handoff ownership + `park_current_and_unlock` 原语）。

...
这些是语言级根本决策，比普通设计原则优先。

1. **编译语言**。NC 是编译语言，不是解释型语言。允许为开发便利提供 `run` 子命令模拟解释执行，但语义上始终先编译后执行。
2. **Go 级别的性能**。NC 的性能目标对标 Go：允许胖指针、间接调用、GC 分配等运行时开销，不追求 C / Rust 级零开销抽象。
...
我们要 Go 风格的 N:M 绿色线程（green thread），由 `spawn fun() -> void {}` 关键字启动。

- `spawn` 是关键字，后跟一个 `fun() -> void` 闭包表达式（零参数，返回 void）。不接受非 void 返回值闭包、不接受函数调用或变量（v1 只接受直接 FunctionExpr）。`spawn` 是语句，不阻塞调用端。
- 编译器 lowering 只生成 `__nc_spawn(fn_ptr, env_ptr)`，G 分配、计数、run queue、worker wake 全由 ncrt 内部在 `scheduler_lock` 下完成。
- `main` 在某个 worker 上以 green thread 运行；进程在 `main_done && user_g_count == 0 && run_queue empty && timer_heap empty` 后由主控制流 exit。
- 调度器实现为 ncrt C runtime：M 个 OS worker（M = NumCPU）从全局 run queue 取 G 执行。所有共享状态（run queue / timer heap / 计数 / 检测）由 `scheduler_lock` 一致保护。
- 每个 green thread 固定 64KB 栈 + guard page；上下文切换用汇编 save/restore callee-saved registers（按 SysV / Win64 ABI 分开实现）。
- G 状态机：G_RUNNABLE / G_RUNNING / G_WAIT_MUTEX / G_WAIT_TIMER / G_DEAD。
- 合作式时间片 yield：编译器仅在函数入口插入检查。loop backedge 仅做 GC safepoint，不做普通调度 yield。
- 同步用标准库 `sync.Mutex`（handoff ownership + `park_current_and_unlock` 原语。park 返回时 G 必定已通过 handoff 拥有锁）。
- 阻塞模拟用 `sleep`（timer 优先队列，调度器每轮检查过期 timer）。
- GC STW 通过完全合作式 safepoint 实现（函数入口 + loop backedge + alloc 点），v1 不做信号/APC 异步抢占。
- 死锁检测在 `scheduler_lock` 下读一致快照：run queue 空、无 G_RUNNING 且所有 live G 均 WAIT_MUTEX/WAIT_TIMER 且无待触发 timer 时，deadlock panic。
- v1 不做 `go` 关键字（未来可作为语法糖）、channel/select、TCP netpoller、工作窃取、动态栈增长、loop backedge 时间片 yield。

why：并发是现代系统语言的核心能力；Go 的 goroutine 模型经过工业验证；case 驱动最小 v1 先覆盖 spawn + mutex + sleep + 死锁检测 + 合作式 GC。`spawn fun() { ... }` 每个闭包必须显式写全，使 green thread 创建有意图、不廉价。未来可用 `go f(args)` 脱糖追加简洁性。

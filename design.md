...
- 默认值可使用无调用的值构造表达式，包括 literal、前序参数引用、struct literal、slice literal、`map[K,V]()` 构造、具名函数值、已实例化泛型函数值、匿名函数值和显式类型转换。
- 默认值不能包含普通函数调用、方法调用或任何可错操作（`??`、`!!`、`is err`）。
- `map[K,V]()` 是默认值中允许的内建 map 构造；显式类型转换是值构造，不按普通函数调用处理。

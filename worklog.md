...
- 2026-06-25: 实现 Phase 2 — Win64 上下文切换：新增 ncrt_switch_win64.S（__nc_g_switch：保存/恢复 GPRs(RBX/RBP/RDI/RSI/R12-R15/RSP) + XMM6-XMM15 + ret；__nc_g_entry_trampoline：pop G 指针 → call entry_fn(entry_arg) → exit(0)）；ncrt.c 新增 __nc_g_init_stack（构造初始 48 字节帧：ret addr→trampoline + G 指针 + 32B shadow space）；build 系统：compiler/ncrt.py 新增 _cached_switch_obj + build_ncrt_objs 返回 [ncrt.o, ncrt_switch.o]，llvm_codegen.py / nc.py / tests 适配。测试 test_phase2.c：main→G1(A)→G2(B)→G1(C)，三跳全通，exit(0)。回归 346/346 语言 + 64/64 pytest。why：证明汇编上下文切换可在 Win64 正常工作；__nc_g_switch 与 trampoline 双组件完整闭环。

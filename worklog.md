...
- 2026-06-25: 去掉 slice 参数 ptr 的自动 noalias。原因：NC 语言允许同一个 slice 传给多个参数（如 f(xs, xs)），noalias 是 LLVM 优化器契约声明指针独立，传入同一个 slice 违反契约，LLVM 可能在未来某版本或优化配置下生成错误代码。改动：declare_function 中删除 slice_ptr_indices 收集和 noalias 添加，保留 slice 拆参 ABI（ptr, len, cap 独立参数），iface thunk / function value thunk 的 adapter ABI 不变。新增 case_393_slice_param_alias.nc 别名回归测试，验证同一 slice 传两个参数时语义正确。验证：python tests/test_language_cases.py 344/344（新增 case_393）；python nc.py run test_cases/case_393_slice_param_alias.nc 输出 2；python -m pytest tests/test_llvm_backend.py tests/test_type_ref.py tests/test_builtin_boundary.py tests/test_projects.py -q 64 passed, 1 skipped。

- 2026-06-25: 预备设计并发模型：通过 grill 流程逐项决策 M:N 绿色线程 v1 边界。设计见 docs/concurrency.md，design.md 同步摘要。v1 覆盖 go 关键字、汇编上下文切换、M 个 worker + 全局 run queue、合作式 yield（函数序言检查）、64KB 固定栈、main 等待、all-asleep 死锁检测、sync.Mutex 标准库、sleep timer yield、GC STW 窄化抢占。v1 不做 channel/select、TCP netpoller、工作窃取、动态栈增长。why：先通后优，case 驱动 v1 最小可行集。验证：文档仅设计阶段，未涉及实现。

- 2026-06-25: 根据 CR 反馈修正 concurrency.md 九处缺陷：① main 改为作为 green thread 计入 live_count，worker 不再调 exit(0)，由 runtime 主控制流在 main_done && user_g_count==0 后退出；② 新增 G 状态机（RUNNABLE/RUNNING/WAIT_MUTEX/WAIT_TIMER/DEAD）；③ Mutex 改为 internal spinlock + handoff ownership，消除丢唤醒；④ Timer 改为每轮调度都检查，消除饥饿；⑤ GC safepoint 改为完全合作式（函数入口 + loop backedge + alloc 点），删除信号/APC 异步抢占；⑥ 栈加 guard page；⑦ x64 上下文切换按 SysV / Win64 ABI 分开列出非易失寄存器（Win64 补 RDI/RSI/XMM6-15）；⑧ 明确栈对齐和 Windows shadow space 要求；⑨ 死锁检测条件改为 all live G 阻塞且无 timer。同步更新 design.md 摘要。why：原设计有致命竞态和实现级缺陷，需先修设计再动手实现。
...
- 2026-06-25: 第三轮 CR 修正 concurrency.md 九处：① 删除 live_count，保留 main_done + user_g_count，少一个概念；② SpawningStmt 统一为 SpawnStmt；③ spawn 闭包 v1 只允许 fun() -> void，不接受非 void 返回值；④ 明确 spawn 只接受直接 FunctionExpr，不接受 work()/make_closure()；⑤ 新增 scheduler_lock 保护全部调度器共享状态，user_g_count++ 与 enqueue 在锁内原子可见；⑥ user_g_count-- 后若 main_done && count==0 则 broadcast worker cv，main_done=true 时也 signal；⑦ shutdown/deadlock 检测在 scheduler_lock 下读一致快照；⑧ park_current_and_unlock 明确返回后置条件：当前 G 已通过 handoff 拥有 mutex；⑨ worklog 按时间顺序重新排列。同步更新 design.md 摘要。why：第三轮 CR 收窄实现级细节（锁协议、唤醒广播、一致快照）；设计已可开工实现 v1 runtime。

- 2026-06-25: 实现 Phase 1 — ncrt G 结构与栈分配：ncrt.h 新增 nc_g_state 枚举（RUNNABLE/RUNNING/WAIT_MUTEX/WAIT_TIMER/DEAD）、nc_green_thread 结构体（Win64 callee-saved + xmm save area + entry fn/arg）、NC_G_STACK_SIZE(64KB)/NC_G_GUARD_SIZE(4KB)；ncrt.c 新增 __nc_g_alloc（VirtualAlloc reserve/commit + guard + 16-byte 对齐）/ __nc_g_free（VirtualFree）+ Linux mmap/mprotect fallback。C 测试 test_phase1.c：单次分配、1000 次批量分配、对齐校验、非重叠校验、全释放；全部通过。语言回归 346/346。why：先让 G 能活能死，不接调度器；验证栈分配 16-byte 对齐和无重叠。

- 2026-06-25: Phase 1.1 — 加固 G 结构：新增 __nc_page_size() 运行期获取页大小取代硬编码 4096；NC_G_GUARD_PAGES=1 替代 NC_G_GUARD_SIZE 硬宏；nc_green_thread 新增 stack_size/guard_size 字段；xmm_save 声明为 16-byte 对齐（MSVC __declspec(align(16)) + GCC __attribute__((aligned(16)))）；__nc_g_alloc 用 page_size * NC_G_GUARD_PAGES 计算 guard，__nc_g_free 用 g->guard_size + g->stack_size 释放；rsp 注释明确为 Phase 1 占位（Phase 2 构造初始 call frame）。test_phase1.c 新增 xmm_save 对齐校验、stack_size/guard_size 字段校验。全部通过，回归 346/346。why：避免 4096 硬假设在未来平台失效；xmm 对齐为 Win64 movaps 准备。

- 2026-06-25: 实现 Phase 2 — Win64 上下文切换：新增 ncrt_switch_win64.S（__nc_g_switch：保存/恢复 GPRs(RBX/RBP/RDI/RSI/R12-R15/RSP) + XMM6-XMM15 + ret；__nc_g_entry_trampoline：pop G 指针 → call entry_fn(entry_arg) → exit(0)）；ncrt.c 新增 __nc_g_init_stack（构造初始 48 字节帧：ret addr→trampoline + G 指针 + 32B 预留区）；build 系统：compiler/ncrt.py 新增 _cached_switch_obj + build_ncrt_objs 返回 [ncrt.o, ncrt_switch.o]，llvm_codegen.py / nc.py / tests 适配。测试 test_phase2.c：main→G1(A)→G2(B)→G1(C)，三跳全通，exit(0)。回归 346/346 语言 + 64/64 pytest。why：证明汇编上下文切换可在 Win64 正常工作；__nc_g_switch 与 trampoline 双组件完整闭环。

- 2026-06-25: Phase 2.1 — 加固上下文切换：① ncrt.c 新增 13 条 _Static_assert 保护所有汇编 hardcoded offset（rsp/rbx/... /xmm_save/entry_fn/entry_arg），字段重排编译期报错而非静默错位；② test_phase2.c 新增寄存器 pattern 测试：G1 设 RBX/R12/R15=0xDEADBEEFCAFE0001，切 G2（clobber 为 0xBAADF00D），切回 G1 断言仍为原始值，验证 GPR 轮转保存/恢复正确；③ __nc_g_init_stack 注释修正：初始 32B 是 padding/reserved 区，非 entry_fn shadow space（trampoline 自行分配）；④ trampoline call exit 前补 sub rsp,32，满足 Win64 caller shadow space；⑤ build_ncrt_objs 按 target 选择 switch assembly 文件，linux-x64 暂时明确报「not implemented」；⑥ nc.py _ncrt_obj 改名为 _ncrt_objs。回归 346/346 语言 + 64/64 pytest + Phase1/Phase2 C test 全通过。why：offset 硬编码是未来最大静默 bug 源；寄存器 pattern 测试补全 Phase 2 的语义证明（不只是「跑通流程」而是「寄存器内容无损」）。
...
- 2026-06-27: 修复 Stage 1 泛型擦除范围过宽导致的回归：仅对无约束、无默认参数、参数/返回均为类型参数且函数体直接返回参数的 identity-shaped 泛型函数走 erased 路径，其余泛型函数继续使用既有单态化路径，避免 Ord、默认参数、泛型函数值等 Stage 2+ 能力被提前擦除；同时修复 keep-objs 分模块构建中 scheduler 注入访问空 main 声明 blocks 的问题。验证：python -B -m py_compile compiler/generics.py compiler/llvm_codegen.py；python tests/test_language_cases.py 通过 354/354；python tests/test_stdlib.py 通过 74/74；python -m pytest tests/test_llvm_backend.py tests/test_type_ref.py tests/test_builtin_boundary.py tests/test_projects.py -q 通过 64 passed, 1 skipped；case_170 LLVM IR 确认三种类型共享单个 erased_id 函数。

- 2026-06-27: 预备实现 Stage 2 泛型类型擦除：支持 `min2[T types.Ord](a: T, b: T): T { if a < b { a } else { b } }` 走 erased 路径，描述符新增 `lt` 函数字段，LLVM lowering 将 `a < b` 改为 `desc.lt(a, b)` 并按结果返回对应 raw slot。why：在 Stage 1 identity 链路稳定后推进第一个约束操作，验证 descriptor-driven operation 的最小闭环。

- 2026-06-27: 完成 Stage 2 泛型类型擦除最小链路：`min2[T types.Ord]` 形态进入 erased 路径，erased 函数新增内部 descriptor raw 参数，调用端生成 `{size, lt}` descriptor 常量和类型专用 `__nc_erased_lt_*` helper，LLVM body 通过 descriptor `lt` 选择返回 raw slot；Stage 1 identity 仍共享单个 erased 函数，其余泛型能力继续走既有单态化路径。验证：python -B -m py_compile compiler/erase_generics.py compiler/generics.py compiler/llvm_codegen.py compiler/llvm_function.py；python -B nc.py run test_cases/case_170_generics_identity.nc；python -B nc.py run test_cases/case_276_generic_ord_constraint.nc；python -B tests/test_language_cases.py 通过 354/354；python -B tests/test_stdlib.py 通过 74/74；python -B -m pytest tests/test_llvm_backend.py tests/test_type_ref.py tests/test_builtin_boundary.py tests/test_projects.py -q 通过 64 passed, 1 skipped；case_276 LLVM IR 确认 i32/f64 调用同一个 erased_min2，并分别传入 desc_min2_i32 / desc_min2_f64。

- 2026-06-27: 通过 grill 流程收束泛型擦除设计并更新文档：泛型函数默认改为类型擦除 + 类型专属固定布局 TypeDesc，泛型 struct 继续单态化；T 在 erased IR 中永远是 raw slot pointer，slice/array/map 在泛型函数内保持 opaque 句柄并通过 helper 访问元素；TypeDesc 固定包含 size/align/copy/zero/eq/lt/hash/trace，helper 统一接收 desc，自身不支持的能力填 trap helper；erased T slot 需要 typed root 以保证 GC trace；types.Hash 隐含 types.Eq，types.Ord 只要求 strict ordering 的 lt 且不隐含 Eq；struct ordering 只保留 __lt__，删除 __le__/__gt__/__ge__ 作为设计边界；[N]T 文档改为长度进入类型的数组对象句柄。同步 docs/generics-implementation.md、docs/generics.md、design.md。本轮仅文档设计收束，未改实现，未运行回归。

- 2026-06-27: 预备实现泛型擦除设计收束的第一刀：先删除 struct ordering 的 __le__/__gt__/__ge__ 用户边界并统一由 __lt__ 派生，同时把现有 erased descriptor 从函数专属命名推进到类型专属 TypeDesc 的可观察骨架；暂不一次性重写 GC typed root 和容器 opaque helper。why：先消除已确定的设计/实现冲突，再逐步替换旧白名单擦除路径，避免大爆炸式改动。

- 2026-06-27: 已完成泛型擦除设计收束第一刀：typecheck 不再把 __le__/__gt__/__ge__ 当作 ordering 特殊方法，struct 的 >、<=、>= 统一由 __lt__ 派生；case_357 改为验证手写 __le__ 不再覆盖派生语义；erased descriptor 调用端全局名从函数专属 __desc_<fn>_<type> 改为类型专属 __nc_typedesc_<type>，并删除前端生成的未使用 __desc_<fn> struct 壳；当前 TypeDesc 仍是最小 {size, lt} 骨架，完整固定布局、typed root、容器 opaque helper 留给后续阶段。验证：python -B -m py_compile compiler\generics.py compiler\erase_generics.py compiler\typecheck.py compiler\llvm_codegen.py；python -B nc.py run test_cases\case_170_generics_identity.nc；python -B nc.py run test_cases\case_276_generic_ord_constraint.nc；python -B tests\test_language_cases.py 通过 354/354；python -B tests\test_stdlib.py 通过 74/74；python -B -m pytest tests\test_llvm_backend.py tests\test_type_ref.py tests\test_builtin_boundary.py tests\test_projects.py -q 通过 64 passed, 1 skipped。

- 2026-06-27: 预备实现 TypeDesc 固定布局骨架：将当前 erased descriptor 从 {size, lt} 扩展为 {size, align, copy, zero, eq, lt, hash, trace}，先只让 lt 被现有 erased min 使用，其余字段填 trap/no-op helper。why：先稳定 ABI 形状，后续才能把 copy、typed root、容器 helper 逐步接入，而不会反复改 descriptor layout。

- 2026-06-27: 已实现 TypeDesc 固定布局骨架：__nc_typedesc_<type> 从 {size, lt} 扩展为 {size, align, copy, zero, eq, lt, hash, trace}，erased min 按固定布局读取 lt 字段；当前 copy/zero/eq/hash 填 trap helper，trace 填 no-op helper，尚未接入实际 copy/typed root/容器 helper。验证：python -B -m py_compile compiler\llvm_codegen.py compiler\llvm_function.py；python -B nc.py run test_cases\case_170_generics_identity.nc；python -B nc.py run test_cases\case_276_generic_ord_constraint.nc；python -B tests\test_language_cases.py 通过 354/354；python -B tests\test_stdlib.py 通过 74/74；python -B -m pytest tests\test_llvm_backend.py tests\test_type_ref.py tests\test_builtin_boundary.py tests\test_projects.py -q 通过 64 passed, 1 skipped。
